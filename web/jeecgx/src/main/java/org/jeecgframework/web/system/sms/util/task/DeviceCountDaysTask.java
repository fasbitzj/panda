...
/**
 * Created by jiang.zheng on 2018/1/16.
 */
@Component("DeviceCountDaysTask")
public class DeviceCountDaysTask implements Job {
    ...
    // 每次检查前多少天到现在的数据，缺失则统计
    private static final int BEFORE_DAYS_COUNT = 3;
    ...
    private void run() {
        logger.info("-------- DeviceCountDaysTask run start ----------");
        deviceCountDay();
        logger.info("-------- DeviceCountDaysTask run end ----------");
    }

    /**
     * 按天统计
     */
    private void deviceCountDay() {
        try {
            ...
            int i = 0;
            do {
                startDateTime = CalendarUtil.daysStartTime(calendar.getTime());
                endDateTime = CalendarUtil.daysEndTime(calendar.getTime());
                //检查是否已统计当前数据
                if (!checkIfExistForDays(startDateTime)) {
                    List<Map<String, Object>> list = deliverReturnCount(startDateTime, endDateTime);
                    deviceCountService.batchSave(mapToDaysEntities(list));
                } else {
                    logger.info(startDateTime + " 设备数据已统计，跳过当前时段");
                }
                calendar.add(Calendar.DAY_OF_MONTH, -1);
                i++;
            } while (i<=BEFORE_DAYS_COUNT);
        } catch (Exception e) {
            logger.error("按天统计设备数异常", e);
        }
    }
     ...
     * @return
     */
    private boolean checkIfExistForDays(String startDateTime) {
        Long count = deviceCountService.getCountForJdbcParam("SELECT COUNT(1) FROM oms_device_count_day WHERE date_days=?", new Object[]{startDateTime});
     ...
     * @return
     */
    private List<DeviceCountDayEntity> mapToDaysEntities(List<Map<String, Object>> list) {
        List<DeviceCountDayEntity> entities = new ArrayList<>();
        DeviceCountDayEntity entity;
        for (Map<String, Object> m : list) {
            entity = new DeviceCountDayEntity();
            ...
            entity.setDateDays(DateUtil.defaultDate((String) m.get("dateHours")));
