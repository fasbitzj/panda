...
public class CgReportController extends BaseController {
	...
	private static final Logger logger = Logger.getLogger(CgReportController.class);
...
//		List<String> paramList = (List<String>) cgReportMap.get(CgReportConstant.PARAMS);
		Map<String, Object> paramDefault = (Map<String, Object>) cgReportMap.get(CgReportConstant.PARAMS_DEFAULT);
		...
		if (null != paramDefault && !paramDefault.isEmpty()) {
			for (Map.Entry<String, Object> entry: paramDefault.entrySet()) {
				String value = request.getParameter(entry.getKey());
				if (StringUtil.isEmpty(value)) value = entry.getValue() + "";
				querySql = querySql.replace("${"+entry.getKey()+"}", value);
			}
		} else{
