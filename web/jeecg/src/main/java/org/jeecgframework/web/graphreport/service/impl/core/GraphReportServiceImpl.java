...
//	}
	...
	public List<Map<String, Object>> queryByCgReportSql(String dbSource,String sql, Map params,
														int page, int rows) {
		sql = handleElInSQL(sql, params);
		String querySql = getFullSql(sql,params);
		log.info("-------------动态报表功能--------querySql--" + querySql);
		List<Map<String,Object>> result = null;
		if(page==-1 && rows==-1){
			result = DynamicDBUtil.findList(dbSource, querySql);
		}else{
			result = DynamicDBUtil.findList(dbSource, querySql, page, rows);
		}
		return result;
	}
